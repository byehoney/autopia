...
Page({
    ...
    onLoad: function (e) { // 页面渲染完成
        ...
        if (e.scene != undefined) {
            ...
            if (scene) { // 预览模式 scene: 第一个数字是store_id(此处需排除)
                tuan_id = scene.split(',')[1];
                team_id = scene.split(',')[2];
                item_id = scene.split(',')[3];
                type = scene.split(',')[4];
                getApp().globalData.share_uid = scene.split(',')[6];
                getApp().globalData.shareType = scene.split(',')[7];
                preview = 1;
            }
        } else { // 正常模式
            tuan_id = e.tuan_id;
    ...
    onShow: function () {
        ...
        // common.tuanPost('webapp.php?c=tuan&a=tuan_info&tuan_id=' + that.data.tuan_id + '&team_id=' + that.data.team_id + '&item_id=' + that.data.item_id + '&type=' + that.data.type + '&preview=' + preview, '', "joinData", that);
        let url = '/pages/join/index?tuan_id=' + this.data.tuan_id + '&team_id=' + this.data.team_id + '&item_id=' + this.data.item_id + '&type=' + this.data.type;
    ...
    onShareAppMessage: function () {
      const { show_share_img, share_img } = app.globalData;
        ...
        return {
            ...
            path: '/pages/join/index?tuan_id=' + this.data.tuan_id + '&team_id=' + this.data.team_id + '&item_id=' + this.data.item_id + '&type=' + this.data.type + "&share_uid=" + getApp().globalData.my_uid + "&shareType=1",
    ...
    _shareFriendsCircle: function () {
        ...
        let data = {
            ...
            id: that.data.tuan_id + ',' + that.data.team_id + ',' + that.data.item_id + ',' + that.data.type,
        ...
        wx.request({
            url: common.Url + '/app.php?c=qrcode&a=share_ewm' + '&store_id=' + common.store_id + '&request_from=app&wx_type=' + common.types + '&wxapp_ticket=' + ticket,
            header: {
    ...
    */
    ...
    creatPost: function () {
        ...
        var canvasData = { // 画布数据
            ...
            bgPath: '../../images/tuan_post_bg.png', // 海报背景图
