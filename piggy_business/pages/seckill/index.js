...
Page({
    ...
    onLoad: function (e) {
        ...
        if (e.scene != undefined) { // 预览模式 
            ...
            if (scene) {
                ...
                id = arr[1];
                preview = 1;
                if (!e.uid && "undefined" != typeof arr[2]) {
                  e.uid = arr[2];
                }
    ...
    gopay: function () {
        ...
        data.send_other = 0;
        data.sku_id = that.data.seckillInfo.sku_id;
        data.custom = [];
        data.quantity = 1;
    ...
    payNow: function () {
        var that = this;
        clearTimeout(publicFun.timer);
        wx.navigateTo({ url: '/pages/payment/index?order_no=' + that.data.allInfo.order_info.order_no })
    },
    // 原价购买
    ...
    onShareAppMessage: function () {
       ...
       return getApp().shareGetFans(title, '', '/pages/seckill/index', 1, image, `&id=${that.data.id}&uid=${f}`);
    },
    /*
    *
    ...
    */
    ...
    creatPost: function () {
        ...
        var canvasData = { // 画布数据
            ...
            // 图片数据
            bgPath: BASE_IMG_URL+'images/seckill_post_bg.png', // 海报背景图
    ...
    drawCanvas: function () {
        ...
        canvas.roundBorderRect({
            ctx,x:left + innerLeft + 40, y:positionY + imgH + 120, h:0.1, w:imgH - 80, r:0,border:"#eeeeee"
        })
