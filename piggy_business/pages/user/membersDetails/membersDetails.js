...
Page({
  ...
  onLoad: function(options) {
    ...
    common.post("app.php?c=ucenter&a=share_info", data, function callBack(res) {
      if (res.err_code == 0) {
        //百分比
        let progress = Number(res.err_msg.now_num) * 100 / Number(res.err_msg.num);
        let description = res.err_msg.description.split("↵");
        that.setData({
  ...
  showCanvasWind() { //生成海报
    ...
    if (this.data.lafenImgurl && this.data.lafenImgurl != "") { //防止二次生成
      this.setData({
        haibaoCanvas: true
      })
      return false;
  ...
  makeCanvas() {
    ...
    if (name.length > 20) { //商品名字长度兼容
      let name_a = name.slice(0, 20) + "...";
      // let name_a = name;
