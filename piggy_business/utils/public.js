    ...
    if (result.err_code == 0) {
      ...
      if (type == 'add_cart') {
        if (result.err_msg.product.union_ticket_code != '') {
          return publicFun.warning('此商品不能加入购物车', that);
        }
      }
...
publicFun.payment = function(that, e, page_name) { //去支付
  ...
  function payment(result) { //去支付
    ...
    if (result.err_code == 0) {
      ...
      publicFun.paymentGo(order_no, page_name)
    }
  };


};
publicFun.paymentGo = function(order_no, page_name) { //去支付
  if (page_name) {
    wx.navigateTo({
...
publicFun.operation = function(that, e, plus) { //购物车页面操作
  ...
  function shoppingNum(result) {
    ...
    if (result.err_code == 0) { //操作成功
      ...
      //商品库存不足或者其情况设置购物车商品数量为1
      if (plus == 'input') {
        let goods_list = shoppingCatData.now_physical_product_list; //正在操作商品列表
        goods_list[index].pro_num = 1;
        that.setData({
          'shoppingCatData': shoppingCatData
        })
      }
...
publicFun.isActiveNew = function(that, isEditActive) { //活动选中状态
  var cart_list = that.data.shoppingCatData.now_physical_product_list;
  let sold_price = 0; //初始购物车商品总售价
  let shop_catnum = 0; //初始购物车商品总数量
  that.data.shoppingCatNum = 0;
  let shop_totalnum = 0;
  for (var i = 0; i < cart_list.length; i++) {
    if (cart_list[i].isActive == 1) {
...
publicFun.choiceShoppingNew = function(that, e, check_type) { //新增购物车按钮选择逻辑
...
publicFun.editChoiceShopping = function(that, e) { //编辑购物车选择按钮
  /***
   * belong_to_physical
  ...
  var cart_all_list = data.cart_list; //当前购物车下所有商品
  let cart_list = []; //购物车不同类目下商品
  ...
  // console.log('data-cart-list', cart_all_list);
  // console.log('data-alldata', data)
  // console.log('data-old', that.data.shoppingCatData)
  that.setData({
...
publicFun.addressSave = function(that, e, go) { //提交地址
  ...
  if (!that.data.user_address.lng || !that.data.user_address.lat) {
    that.setData({
      positionError: true
    })
  ...
  if (address_detail.length === 0) {
    return publicFun.warning('请填写街道门牌信息', that)
  }
...
publicFun.chooseLocation = function(that, cb) {
  wx.chooseLocation({
    ...
    fail: function(res) {
      wx.showModal({
        title: '',
        content: '由于您关闭位置的授权暂时无法定位，请重新开启定位',
        confirmText: '授权开启',
        cancelText: '取消',
        success(res) {
          if (res.confirm) {
...
publicFun.localExpress = function(that, integralChange) {
   ...
   * **/
  //只校验不是从待支付订单过来的
  if (that.data.paystatus != "waitpay") {
    if (now_physical.local_type == 1) {
      if (typeText == 'order_money') {
        //满减>订单价
        if ((now_physical.delivery_min_amount * 1 > total)) {
  ...
  // console.log("走 获取当前配送的位置，如果位置没有坐标，需要修改当前位置");

  //距离计算
  let currentDeliverShop = calcDistance(that.data.paymentData.wxapp_address, now_physical, that)
  let pay_status = that.data.paymentData.order.status; //1=待支付订单、0=临时单
  if (currentDeliverShop == null || currentDeliverShop == undefined) {
    return;
  }
  currentDeliverShop.support_delivery = pay_status == 1 ? true : currentDeliverShop.support_delivery;
  let pay_postage = pay_status == 1 ? that.data.paymentData.order.postage : currentDeliverShop.current_deliver_fee;
  let show_mail = true;
  if (currentDeliverShop.support_delivery) {
    //重量配送邮费
    if (now_physical.localfeetype == 1) {
      let fee = now_physical.weight_delivery_fee ? now_physical.weight_delivery_fee : 0;
      that.setData({
        postage: true,
        'paymentData.order.postage': fee
      })
    } else {
      //距离配送邮费
      that.setData({
        no_postage_text: "",
        postage: true,
        'paymentData.order.postage': pay_postage
      })
    }

    if (integralChange == 'inteChanged') {
    ...
    show_mail = false;
    that.setData({
      no_postage_text: "当前地址不在当前门店配送范围内，请选择切换地址或门店",
      postage: false,
  ...
  //重量计算
  // console.log('localfeetype ----', now_physical.weight_delivery_fee)

  // if (show_mail&&now_physical.localfeetype==1){
  //   let fee = now_physical.weight_delivery_fee ? now_physical.weight_delivery_fee:0;
  //   that.setData({
  //     postage:true,
  //     'paymentData.order.postage':fee
  ...
  //判断门店与当前选择地址的位置的距离，返回是否支持配送,并计算配送费
  function calcDistance(address, shop, that) {
    let distance = getFlatternDistance(address.lat, address.lng, shop.lat, shop.lon);
    if (!address.lat) {
      wx.showModal({
        title: '',
        ...
        success: function(res) {
          wx.navigateTo({
            url: '/pages/user/address/addressEdit?addid=' + address.address_id + '&order_no=' + that.data.paymentData.order.order_no_txt + '&address=' + address.address_id
          })
        }
      })
    ...
    console.log(distance, shop.name)
    if (shop.local_type == 1) { //限定配送距离
      if (shop.delivery_scope_type == 1) { //半径
        shop.support_delivery = shop.delivery_scope * 1 > distance * 1
      } else { //多边形
        shop.support_delivery = IsPtInPoly(address.lng, address.lat, shop.delivery_scope);
      }
      let set_deliver_fee = false; //判断当运费与起步条件都为0的时候
      let hasout_deliver_fee = false; //运费条件达到的时候并且价格大于条件
      console.log("==========", shop.custom_delivery_fee)
      shop.custom_delivery_fee.sort((a, b) => a.consume_amount * 1 > b.consume_amount * 1).forEach(item => {
        if (item.delivery_fee && parseInt(item.delivery_fee) >= 0 && item.consume_amount && parseInt(item.consume_amount) > 0) {
          // set_deliver_fee = true;
          //判断运费条件内运费存在且大于等于0，运费门槛>0的时候进行判断
          if (parseInt(baseMoney) >= parseInt(item.consume_amount)) {
            hasout_deliver_fee = true;
            set_deliver_fee = true;
            //判断总价大于各个门槛条件的时候返回运费条件
            shop.current_deliver_fee = item.delivery_fee * 1;
          }
        }
      })

      if (!set_deliver_fee || shop.current_deliver_fee == undefined || (shop.current_deliver_fee === 0 && !hasout_deliver_fee)) { //如果运费条件不满足设置的情况
        shop.current_deliver_fee = shop.delivery_detault_fee;
      }
    } else { //不限配送距离
      //    小于最小范围，则用最小配送费
      shop.support_delivery = true
      if (distance <= shop.limit_space * 1000) {
        shop.current_deliver_fee = shop.limit_fee
      } else {
        shop.current_deliver_fee = parseFloat(shop.limit_fee) + parseFloat(Math.ceil((distance - shop.limit_space * 1000) / (shop.delivery_beyond_space * 1000)) * shop.delivery_beyond_fee)
      }
      shop.current_deliver_fee = (shop.current_deliver_fee * 1).toFixed(2);
    }
    return shop
  }
...
publicFun.paymentButton = function(that, e) { //支付按钮
  ...
  let payType = e.target.dataset.type;
  ...
  if (that.data.paymentPostage == 'express' && !address_id && !address) {
    that.setData({
      flags: true
    });
          ...
          if (orderInfo.type == 6 && orderInfo.data_tuan_item_id) {
            wx.showModal({
              ...
              complete(e) {
                if (e.confirm) {
                  wx.redirectTo({
                    url: `/pages/join/index?tuan_id=${orderInfo.data_id}&team_id=${orderInfo.data_item_id}&item_id=${orderInfo.data_tuan_item_id}&type=1`
                  })
                } else {
                  wx.redirectTo({
...
publicFun.paymentMoney = function(that) {
  ...
  //应付价格
  that.data.paymentMoney = total + float_amount - that.data.rewardPrice * 1 - that.data.couponMoney * 1 - that.data.integralPricePage * 1 - discount - that.data.presaleReducePrice + postage;
...
publicFun.userCall = function(res, app, that) {
  ...
  if (res.detail['userInfo']) {
    ...
    // 以上新增--------------
    console.log(app.globalData.loginData.type, "app.globalData.loginData.typeapp.globalData.loginData.type")
    let data
    if (app.globalData.loginData.type == 2) {
      ...
      data = {
        ...
        codes: app.globalData.login.code || g_code,
    ...
    function setUserInfo(result) {
      ...
      console.log("result.err_msg.wxapp_ticket", result.err_msg.wxapp_ticket);
      publicFun.addFans();
      ...
      if (store_openid) {
        ...
        if (that) {
          that.setData({
            userLoginUrl: common.Url + `wap/reh5.php?store_id=${app.globalData.store_id}&wxapp_ticket=${result.err_msg.wxapp_ticket}#wechat_redirect`
          })
        }
...
publicFun.loginLoaded = function(e, app, that) {
  ...
  //验证邀请码
  if (app.globalData.loginData.type == 2) {
    common.post('app.php?c=drp&a=check_invite_code&code=' + app.globalData.invite_code, "", res => {
      //todo res未定义

      if (res.err_code == 0) {
        app.globalData.loginData.type = 0
      } else {
        // that.setData({
        //   showLoginModal: true,
...
publicFun.onLoad = function(app, that) { //授权页面
  ...
  that && (that.getCode = function(e) {
    ...
    if (!(/^1[3456789]\d{9}$/.test(app.globalData.new_numInput))) {
      wx.showModal({
        title: '提示',
        content: '手机号有误',
        success(res) {
...
publicFun.formSubmit = function(param) { // 生成下发模板消息所需的formId存于服务器
  ...
  // console.log('formSubmitPublic触发，向服务器存储formId，携带数据为：', param.e.detail.formId)
  // console.log('common.store_id: ', common.store_id)
