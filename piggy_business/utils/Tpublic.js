...
publicFun.detailsGo = function(e) { //跳转详情页面
    ...
    wx.redirectTo({ url: '/pages/details/index?tuan_id=' + tuan_id })
};
...
publicFun.paymentGo = function(order_no) { //去支付
    wx.redirectTo({ url: '/pages/payment/index?order_no=' + order_no });
};
...
publicFun.joinGo = function(e, that) { //参团路由
    ...
    let item_id = e.target.dataset.item;
    let type = e.target.dataset.type;
    wx.navigateTo({ url: '/pages/join/index?tuan_id=' + tuan_id + '&team_id=' + team_id + '&item_id=' + item_id + '&type=' + type });
};
