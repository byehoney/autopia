    ...
    <div class="goodsContainer">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="ruleForm">
            ...
            <el-form-item label="" prop="goodsType">
                <div style="color:#ccc">若需要新增分类，请联系车托帮工作人员添加</div>
            </el-form-item>
            ...
            <el-form-item label="" prop="goodsType">
                <div style="color:#ccc">若需要新增品牌，请联系车托帮工作人员添加</div>
            </el-form-item>
            <el-row>
                <el-col :span="8">
            ...
            <el-form-item label="封面图片：">
                ...
                <el-row class="grey">封面图尺寸750X750像素，不超过1M；点击+可添加多张封面图</el-row>
            </el-form-item>
            ...
            <el-form-item label="库存管理：" v-if="this.specification.length">
                <SkuTable :skusData="specificationFilter" :isEdit="edit" ref="skuInfo"></SkuTable>
            </el-form-item>
            ...
            <el-form-item label="图文介绍：" style="height:450px">
                 <!-- 图片上传组件辅助-->
                <el-upload ref="quillImg" multiple style="height:0" class="quill-upload" :action="baseUrl+'/supplier/goods/upload/imgM'" :show-file-list="false" :on-success="quillImgsuccess" :before-upload="quillImgBefore"></el-upload>
                <!--富文本编辑器组件-->
            ...
            <el-form-item class="center">
                <el-button @click="doSubmit(2)">提交审核</el-button>
                <el-button type="primary" @click="doSubmit(0)">保存到草稿箱</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
<script>
import { serviceList ,getMainClass,getSonClass ,getBrands ,addService ,specificationList ,specificationValues ,addSpecification,submitGoodsInfo} from '../../api/getData';
...
export default {
    ...
    methods:{
        ...
        async reqSonClass(){
            ...
            let res = await getSonClass({ fid:this.ruleForm.mainClass});
            if(res.code == 0){
        ...
        async reqValues(id,name,left){
            ...
            if(res.code == 0){
                if(res.data.length){
                    ...
                    this.$set(this.specification[this.curModeIndex],'leaf',[]);
                }
                // else{
                //     this.$set(this.specification[this.curModeIndex],'id',id);
                //     this.$set(this.specification[this.curModeIndex],'value',name);
        ...
        // addGe(spec,index){
        //     let len = this.specification.filter((item)=>item.value==spec.value).length;
        //     this.$message({
        //         showClose: true,
        //         message: '规格名称重复',
        //         type: 'warning'
        ...
        //     // this.$set(this.specification[index],'leaf',[]);
        //     
        // },
        onEditorChange(e) {
        ...
        async addService(){//添加服务承诺
            if(!this.service){
                this.$message.error('承诺内容不能为空');
                return;
            }
            let res = await addService({service_name:this.service});
            if(res.code == 0){
                this.service = "";
                this.$message({
                    message: '添加成功',
        ...
        addOption(index) {
            ...
            addSpecification({spesc_info:spesc_info}).then((res)=>{
                if(res.code == 0){
                    ...
                    this.$set(this.specification[index],'leaf',size);
                    this.$set(this.specification[index],'left',size);
                    this.$set(this.specification[index],'id',res.data[0].specs_id);
                    // this.specification[index]
                }else{
                    this.$set(this.specification[index],'leaf',[]);
                    this.$set(this.specification[index],'left',[]);
                    this.$set(this.specification[index],'id','');
                    this.$message.error(res.message);
                }
            })
        },
        ...
        async doSubmit(sh_status){
            ...
            if(!this.checkInfos()){
                ...
                let res = await submitGoodsInfo(data);
                if(res.code == 0){
                    this.$message({
                        message: '操作成功',
                        type: 'success'
                    });
                    this.$router.history.push({path:'/goodsManage/goodsStock'})
                }else{
                    this.$message.error(res.message);
                }
            }
