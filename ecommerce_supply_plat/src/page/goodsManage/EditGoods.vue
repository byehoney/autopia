...
<script>
import { serviceList ,getMainClass,getSonClass ,getBrands ,addService ,specificationList ,specificationValues ,addSpecification,submitGoodsInfo,getGoodsInfo,saveGoodsInfo} from '../../api/getData';
...
export default {
    ...
    methods:{
        async reqGoodsInfo(){
            let res = await getGoodsInfo({id:this.$route.query.id,draft:this.$route.query.draft});
            if(res.code == 0){
                ...
                res.data.picture.length&&res.data.picture.map((item,index)=>{
                ...
                res.data.invoices_type&&res.data.invoices_type.split(',').map(item=>{
                    bill.push(item)
                })
